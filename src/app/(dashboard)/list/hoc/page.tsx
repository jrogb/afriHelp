    ...
    <td>
      <div className="flex items-center gap-2">
        <Link href={`/list/it/${item.id}`}>
          {item.role === "Admin" && <FormModal table="it" type="update" data={item} />}
        </Link>
        {item.role === "Admin" && <FormModal table="it" type="delete" id={item.id} />}
      </div>
    </td>
  </tr>
);

const HOCListPage = async ({ searchParams }: { searchParams: { [key: string]: string | undefined } }) => {
  const { page, ...queryParams } = searchParams;
  const p = page ? parseInt(page) : 1;

  // Build the combined Prisma where query
  const query: Prisma.UserWhereInput = {
    role: "HOC", // filter by role
  };
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden text-lg font-semibold md:block">Heads of Company</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch />
          <div className="flex items-center gap-4 self-end">
            {currentUserRole === "Admin" && <FormModal table="it" type="create" />}
